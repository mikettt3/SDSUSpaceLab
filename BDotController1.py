...
try:
    while i<=(runtime-1):
        ...
        # Bdot = (Bxyz[i] - Bxyz[i-1])/dt
        #       dt = t(i) - t(i-1) ~= 50 ms; dt = 50.0
        if i == 0:
            B_dot = [0, 0, 0]
        else:
            B_dot = (imuR[i,1:4]-imuR[i-1,1:4])/((imuR[i,0]-imuR[i-1,0])/1000)
        ...
        # [B] = imu[Bx, By, Bz] 
        # For ease of calculation readability below
        Bx = imuR[i,1]
        By = imuR[i,2]
        Bz = imuR[i,3]
        ...
        for j in range(0,3):
            if T_ctrl[j] > 1:
                ...
                T_ctrl[j] = int(T_ctrl[j]*100)/100 
                # ctrl = ctrl. Trim to sensible values
    
#         print(mu_ctrl.transpose())
        print(T_ctrl.transpose())
        ...
        imuR[i,13:16] = B_dot
        imuR[i,16:19] = mu_ctrl[0:3,0]
        imuR[i,19:22] = T_ctrl[0:3,0]
